...
public interface UserFollowService {
     ...
     * @return 分页的关注用户列表
     */
    @GET("api/user-follows/following/{userId}")
    ...
    /**
     * 查询指定用户的粉丝列表（分页）。
     * 对应后端 GET /api/user-follows/followers/{userId}
     *
     * @param userId 目标用户ID
     * @param pageNum 页码，默认 1
     * @param pageSize 每页大小，默认 10
     * @return 分页的粉丝用户列表
     */
    @GET("api/user-follows/followers/{userId}")
    ...
    /**
     * 检查当前用户是否关注指定用户。
     * 对应后端 GET /api/user-follows/check/{followingId}
     *
     * @param followingId 被检查的目标用户ID
     * @return true表示已关注，false表示未关注
     */
    @GET("api/user-follows/check/{followingId}")
